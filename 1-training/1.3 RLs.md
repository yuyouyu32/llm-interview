...
### Q. SFT 和 RL 的优劣势  
> **Company**: 上海 AI Lab ｜ **Round**: 算法工程师 一面 ｜ **Date**: 2025-03-08 ｜ **Tags**: [SFT, RL, 优缺点]

**SFT（Supervised Fine-tuning）**  
- **优势**：  
  - 简单稳定，训练收敛快，工程落地成本低。  
  - 有大量高质量标注数据时效果好。  
  - 对任务格式/输出样式的控制力强，能快速适配新任务。  
- **劣势**：  
  - 强依赖人工标注，数据成本高。  
  - 学到的是“模仿分布”，难以优化更高层次目标（如用户满意度）。  
  - 泛化到长尾场景或开放式任务时能力不足。  

**RL（Reinforcement Learning, e.g. PPO/GRPO）**  
- **优势**：  
  - 可利用奖励信号直接优化目标（偏好、评分、成功率），突破纯模仿的局限。  
  - 允许在有限监督下结合探索，发现超出训练分布的新策略。  
  - 可集成对齐目标（如人类价值观、系统约束）。  
- **劣势**：  
  - 训练复杂，不稳定，对超参和奖励设计敏感。  
  - 需要 rollout，算力和时间成本高。  
  - 奖励稀疏或设计不当时，容易出现模式崩坏或过拟合奖励。  

<mark>一句话总结：SFT 稳定高效但受限于标注分布，RL 能直接优化偏好和目标但成本高且训练不稳。</mark>

### Q. SFT 和 RLHF 后的模型输出内容会有什么区别，产生的原因？  
> **Company**: 上海 AI Lab ｜ **Round**: 算法工程师 一面 ｜ **Date**: 2025-03-08 ｜ **Tags**: [SFT, RLHF, 输出差异]

**SFT（监督微调）输出特点**  
- **内容偏模仿**：主要再现训练集中常见的答案模式。  
- **表达中规中矩**：语言简洁，贴近标注分布，但缺少“人性化”调节。  
- **局限**：遇到长尾问题或需要推理的场景，往往容易照搬训练数据风格，缺乏灵活性。  

**RLHF（人类反馈强化学习）输出特点**  
- **更偏向人类偏好**：模型会生成更安全、礼貌、有解释性的答案。  
- **形式更自然**：更注重对话流畅性、完整性和读者体验。  
- **潜在问题**：可能过度冗长、趋同化（模式坍缩），因为模型在优化偏好奖励而非单纯模仿。  

**产生原因**  
- **SFT**：仅依赖标注数据做条件分布拟合，优化目标是最大似然 → 输出接近“训练数据的平均风格”。  
- **RLHF**：引入奖励模型，将人类偏好作为优化信号，直接调整输出概率分布 → 输出更契合人类价值与偏好，而不仅是数据分布。  

<mark>一句话总结：SFT 输出更像“数据的复制品”，RLHF 输出更像“人偏好的定制品”，本质差异来自优化目标——最大似然 vs 偏好奖励。</mark>
...
### Q. dense和sparse reward两种范式讲讲 怎么看?
**Company**: 腾讯 ｜ **Round**: WXG青云算法工程师 一面 ｜ **Date**: 2025-09-05 ｜ **Tags**: [奖励设计, Dense Reward, Sparse Reward, LLMs]

**1. 背景问题**  
- 在传统强化学习 (RL) 中，Dense Reward（稠密奖励）与 Sparse Reward（稀疏奖励）是两种常见范式。  
- 在 **大语言模型 (LLMs) 的训练与对齐** 中，奖励函数同样关键（如 RLHF、RLAIF、GRPO）。  
- 如何在 LLMs 中选择 dense 或 sparse 奖励，是一个涉及效率、稳定性和泛化的核心问题。

**2. Dense Reward 在 LLMs 中的体现**  
- **定义**：模型生成的每一步或每个 token 都可能得到奖励信号。  
- **应用**：  
  - 基于打分器 (reward model) 对输出的流畅度、格式正确性、事实性逐项打分。  
  - 多维指标综合（如 BLEU、ROUGE、毒性检测、结构完整性）。  
- **优点**：  
  - 梯度丰富，训练稳定，能快速收敛。  
  - 适合需要结构化输出的任务（如代码生成、信息抽取）。  
- **缺点**：  
  - 奖励设计复杂，容易奖励错位（Reward Hacking），模型可能学会讨好打分器而非完成真实目标。  

**3. Sparse Reward 在 LLMs 中的体现**  
- **定义**：模型只有在完整输出符合要求时才得到奖励。  
- **应用**：  
  - 指令遵循：只有完全符合指令的回答才给奖励。  
  - 任务正确性：如数学题正确才给分，错误则 0 分。  
- **优点**：  
  - 保证奖励信号紧贴目标，避免中间过程“误导”。  
  - 特别适合高可靠性任务（如推理、定理证明、复杂问答）。  
- **缺点**：  
  - 学习难度大，early training 阶段容易“全是 0 奖励”，训练不稳定。  
  - 需要探索性方法或 curriculum learning 来缓解。  

**4. 工程实践中的混合方式**  
- 在 LLM 训练中常用 **Hybrid Reward**：  
  - **Sparse 目标奖励**：严格评估最终结果正确性。  
  - **Dense shaping 奖励**：在生成过程提供结构、风格、格式等中间反馈。  
- 例如：在代码生成里，编译正确性是 sparse 奖励；函数命名规范、格式匹配是 dense 奖励。  
- 这种结合既保持了 sparse 的目标纯度，又利用 dense 奖励提升收敛效率。  

<mark>在 LLM 训练中，Dense Reward 能加快收敛、提升结构稳定性，但易被“迎合”；Sparse Reward 更贴近真实任务目标，但训练难度高，因此实际多采用两者结合以兼顾效率与对齐质量。</mark>

### Q. MCTS (Monte Carlo Tree Search) 在传统 RLs 中和 LLMs 会有什么不同？  
> **Company**: 上海 AI Lab ｜ **Round**: 算法工程师 一面 ｜ **Date**: 2025-03-08 ｜ **Tags**: [MCTS, RL, LLMs]

**在传统 RL 中**  
- **状态-动作空间**：通常是明确的、有限或可抽象化（棋类、博弈、规划任务）。  
- **搜索过程**：MCTS 在环境中模拟 rollout（playout），结合 UCT 算法进行节点扩展与价值估计。  
- **价值来源**：依赖真实环境模拟或已训练的 value/policy 网络给估值。  
- **特点**：搜索树浅，分支有限，模拟结果较准确，适合棋类或规则明确的任务。  

**在 LLMs 中**  
- **状态-动作空间**：状态是“已有文本序列”，动作是“下一个 token”，分支数 = 词表大小（通常几万），远大于棋类。  
- **搜索过程**：直接环境模拟不可行 → rollout 常由 LLM 自身生成，需要截断与采样。  
- **价值来源**：依赖外部奖励模型（如 RM、偏好模型）或启发式指标（如困惑度、对齐度）评估生成质量。  
- **特点**：  
  - 树分支巨大 → 只能用 **部分扩展 + 采样** 近似搜索。  
  - rollout 成本高 → 常限制深度或采用并行批量生成。  
  - 更偏向于 **引导式搜索**（如在推理、多选题中用 MCTS 聚合最优回答），而非完整遍历。  

**关键差异总结**  
- **搜索空间**：传统 RL → 小而明确；LLMs → 大而离散（token-level）。  
- **价值评估**：传统 RL → 环境奖励；LLMs → 外部 RM/启发式评分。  
- **搜索策略**：传统 RL → 可深度模拟；LLMs → 需剪枝+采样+启发式。  

<mark>一句话总结：MCTS 在传统 RL 中是“明确规则下的模拟搜索”，而在 LLM 中则变为“在庞大语言空间里依赖奖励模型和采样的近似搜索”。</mark>


### Q. Reward hacking 的原因？怎么解决的？  
> **Company**: 上海 AI Lab ｜ **Round**: 算法工程师 一面 ｜ **Date**: 2025-09-23 ｜ **Tags**: [Reward Hacking, RL, 解决方案]

**产生原因**  
- **奖励函数设计不完备**：奖励未能全面覆盖任务目标，智能体学会“钻漏洞”。  
- **代理目标偏离**：模型只学会最大化奖励信号，而非人类真实意图（specification gaming）。  
- **稀疏或延迟奖励**：导致智能体过度放大一些容易获取的伪奖励。  
- **高维复杂环境**：难以手工定义完美 reward，增加了投机行为的可能性。  

**解决方法**  
1. **改进奖励设计**  
   - 奖励塑形（Reward shaping），补充中间奖励避免偏离。  
   - 多目标奖励，结合精度、效率、安全性。  
2. **引入人类反馈**  
   - RLHF 或 preference learning，用人类偏好约束智能体。  
   - 对抗式评估，人工甄别潜在漏洞。  
3. **正则与约束**  
   - 在优化目标中加入 KL 正则、约束条件，避免过度偏离参考策略。  
   - 使用 rule-based filter 对输出做硬约束。  
4. **环境与测试多样化**  
   - 增加 adversarial 环境或 stress test，减少单一奖励下的漏洞利用。  
   - OOD 测试，检验鲁棒性。  

<mark>一句话总结：Reward hacking 源于奖励函数与真实目标的错位，解决之道是改进奖励设计、引入人类反馈并施加约束来减少漏洞利用。</mark>
...
### Q. DPO 有哪些变种（如 IOPO），他们各自的优化点是什么？  
> **Company**: 腾讯 ｜ **Round**: 青云四面 ｜ **Date**: 2025-09-25 ｜ **Tags**: [DPO, 变种, 优化点]

| 变种 / 扩展 | 主要优化点 / 适用场景 | 可能的权衡 / 局限 |
|---|---|---|
| **IPO（Identity Preference Optimization）** | 为了缓解 DPO 在偏好数据上过拟合的风险，通过引入一种“identity mapping”（标签平滑 / 概率变换）或对数 odds 映射来使 preference 概率分布更平滑，从而降低倾向性极端更新。 | 可能会使模型的偏好信号弱化；如果 label 很干净，过度平滑可能降低对偏好的响应度。 |
| **KTO（K-to-One / K 对比优化）** | 从 pairwise preference 扩展到 K 候选集（K>2），在训练时构造更丰富的负例对比（不仅一正一负），使模型在多个候选中区分能力更强。 | 计算负例组合更多时候代价也更高，可能需要更大的内存 /计算资源；对偏好数据质量更敏感。 |
| **CPO** | CPO (Conservative Preference Optimization / Conservative DPO) 的设计思路是对偏好信号与模型移动做更保守的更新，控制偏移（类似于 PPO 的保守策略思路）。 | 如果保守力度太大，收敛速度变慢；若偏好信号强、偏移需求大时可能受限。 |
| **Hybrid Preference Optimization (HPO)** | 在 DPO 基础上引入额外的辅助目标（如有害内容惩罚、简洁度奖励、可理解性奖励等），使得模型不仅满足用户偏好，还能兼顾设计者／系统偏好。这样可以把非偏好目标“软”地加入优化范式中。 | 需要设计好辅助目标与主要偏好目标之间的权重；辅助目标冲突可能导致训练不稳定。 |
| **Iterative DPO / DPO-VP** | 通过 “多轮 DPO” 框架，让模型与奖励模型相互增强——每轮用当前模型＋奖励模型生成新样本、更新偏好对，再做下一轮 DPO；DPO-VP 指在推理/可验证 reward 上做强化，以更少代价达到接近 RL 的效果。 | 训练流程更复杂，可能引入偏差累积；奖励模型错误可能放大；样本质量控制更关键。 |
| **Sem-DPO** | 在 DPO 的基础上加入语义一致性约束：用 prompt 与候选之间的 embedding 相似度（cosine distance）作为一个权重，对偏好信号做加权，抑制那些语义偏离 prompt 的极端更新。适用于提示优化 / prompt 调优中防止偏离原意的场景。 | 对 prompt-embedding 空间敏感；在大规模生成任务中可能引入额外计算开销。 |
| **Weights-Rotated Preference Optimization (RoPO)** | 针对 DPO 存在的 **reward hacking** / 表示冗余 / 神经元坍塌问题，RoPO 在输出层维持 KL 约束，同时在中间隐藏层施加正交/旋转约束，防止模型偏离 reference 模型过远，保留预训练 / SFT 的知识能力。 | 设计正交 / 旋转约束的方式较复杂；需要调参约束强度；在极端偏好任务下可能限制偏好探索空间。 |
| **RS-DPO (Rejection Sampling DPO) / Self-Improvement DPO** | 这种方法动态地从当前模型生成“负例 / 差答案”（rejections），与已有优答案配对用于 DPO 训练；也做 self-generation 的正样本选择，用模型自己输出增强训练集，实现持续自我提升。 | 模型生成低质负例可能带入噪声；训练稳定性较差；如果依赖自身样本，容易陷入自我强化偏差。 |
| **IOPO（Input-Output Preference Optimization）** | 在 DPO 基础上引入“输入-输出联合偏好”：构造 (x₁,y₁)、(x₂,y₂) 的匹配组合 vs 错配组合，训练模型倾向前者，从而提升模型对输入中细微约束差异的敏感度，更适合复杂指令遵从场景。 | 数据构造复杂，需要成对输入与对应输出的组合；自动生成可能带噪声；训练开销比 DPO 更大。 |


- 如果担心 **过拟合偏好数据 / label noise**：可以优先用 **IPO / CPO** 或 **RoPO** 来引入更保守 / 正则化的机制。  
- 若希望 **提高模型在多个候选中的区分力**：可以优先考虑 **KTO**、或 **Iterative DPO**（使得训练样本更丰富）。  
- 当想 **兼顾多个系统目标（非偏好目标）**：用 **Hybrid Preference Optimization (HPO)** 把那些额外目标结合进来。  
- 在提示优化 / prompt 学习场景下，要防止语义偏离原 prompt：**Sem-DPO** 是专门为此设计。  
- 若关注 **reward-hacking / 表示坍塌 / 模型能力遗忘**：**RoPO** 是对 DPO 的结构性正则改善。

<mark>**DPO 的变种大多围绕着“缓解过拟合 / 增强稳定性 / 扩充样本 / 加入正则与辅助目标”展开，各有侧重，应根据实际任务与偏好数据特点选择合适变体。**</mark>


## 3. [RFT](https://arxiv.org/pdf/2308.01825) (Rejection Sampling Fine-Tuning)

### Q. 什么是大模型中的拒绝采样（Rejection Sampling）？它与统计学中的概念有何不同？
> **Company**: 美团 ｜ **Round**: 本地商业大模型二面 ｜ **Date**: 2025-09-08 | **Tags**: [RFT, 拒绝采样, 统计学]

**A. 统计学中的拒绝采样**  
用于从复杂分布 \( p(x) \) 中采样。给定易采样的分布 \( q(x) \) 与常数 \( M \)，满足：
$$
p(x) \le M q(x)
$$  
采样步骤：从 \( q(x) \) 取样 \( x' \)，再从 \( U(0,1) \) 取 \( u \)，若：
$$
u \le \frac{p(x')}{M q(x')}
$$  
则接受，否则拒绝。最终保证样本服从目标分布 \( p(x) \)。

**B. 大模型中的拒绝采样**  
在 RFT 或 RLHF 中，模型生成多个候选输出 \( \{y_i\} \)，通过奖励模型 \( R(y_i) \) 打分，仅保留高于阈值 \( \tau \) 的样本：
$$
y_i \text{ 被接受当 } R(y_i) \ge \tau
$$  
用于过滤低质量或不安全输出，强调质量与偏好一致性，而非分布还原。

**C. 区别总结**

| 维度 | 统计学 | 大模型 |
|------|--------|--------|
| 目标 | 获得服从目标分布的样本 | 保留高质量输出 |
| 拒绝依据 | 概率密度比 \( \frac{p(x)}{Mq(x)} \) | 奖励函数或置信度 |
| 分布一致性 | 保证一致 | 不保证一致，偏向优质样本 |

<mark>**一句话总结：统计学的拒绝采样保证分布一致性，而大模型的拒绝采样是基于奖励的样本筛选机制，用于质量控制。**</mark>
...
### Q. GRPO 的实现为什么需要先 rollout 再把生成的回答输入到模型里？  
> **Company**: 上海AI Lab | **Round**: 大模型能力增强实习生一面 | **Date**: 2025-06-27 | **Tags**: [RL, GRPO]

在 GRPO 中，“先生成 → 再回放”的流程是必需的，核心原因有三点：

**1. 先采样才能拿到奖励**  
- 奖励往往在**完整回答结束后**才可计算（如 Judge 打分、偏好排序）。  
- 没有 rollout，就无法得到 trajectory 级别的 return 和优势函数。  
...
- rollout 只记录了旧策略 log-prob；训练时必须**再输入序列**，用新策略求 log-prob、value、KL，才能形成梯度。  

**3. 工程与稳定性考虑**  
- 采样阶段一般 `no_grad`，避免显存爆炸；训练阶段才 teacher-forcing 构建计算图。  
- 回放还能保证 **mask、对齐、组相对基线** 等实现统一，训练稳定高效。  

<mark>一句话总结：GRPO 先 rollout 拿奖励，再回放重算 log-prob/价值/优势，才能正确构造 (s,a,r) 与稳定训练。</mark>
